...
public class UserAction {
	...
	public void toupdate(@Attr(Webs.ME) Sys_user user, @Param("userid") long userid, HttpSession session, HttpServletRequest req) {
		/*
		 * Sys_user obj = sysUserService.fetch(userid); Sys_unit unit =
		 * sysUnitService.detailByName(obj.getUnitid()); req.setAttribute("obj",
		 * obj); req.setAttribute("unit", unit);
		 * obj.setPassword(Lang.digest("MD5",
		 * Strings.sNull(obj.getPassword()).getBytes(),
		 * Strings.sNull(obj.getSalt()).getBytes(), 3)); boolean self = false;
		 * if (user.getLoginname().equals(obj.getLoginname())) { self = true;
		 * req.setAttribute("self", self); } Hashtable<String, String> hashrole
		 * = new Hashtable<String, String>(); Sql sql =
		 * Sqls.create("select roleid,'wiz' from sys_user_role where userid = '"
		 * + obj.getUserid() + "'"); hashrole =
		 * sysUserRoleService.getHTable(sql); LinkedList<Sys_role> list = null;
		 * LinkedList<Sys_role> rolelist = null; LinkedList<Sys_unit> unitlist =
		 * null; int sysrole = 0; Condition unitc; Condition rolec; if
		 * (user.getSysrole()) // 判断是否为系统管理员角色 { unitc =
		 * Cnd.wrap("id in (select unitid from sys_role ) order by location,id asc"
		 * ); rolec =
		 * Cnd.wrap("unitid is null or unitid='' order by location,id asc");
		 * sysrole = 1; } else { unitc =
		 * Cnd.wrap("id in (select unitid from sys_role where unitid like '" +
		 * user.getUnitid() + "%') order by location,id asc"); rolec = Cnd.wrap(
		 * "(unitid is null or unitid='') and id in (select roleid from sys_user_role where userid='"
		 * + user.getUserid() + "') order by location,id asc"); } unitlist =
		 * (LinkedList) sysUnitService.listByCnd(unitc);
		 * req.setAttribute("sysrole", sysrole); Hashtable<String, String> hm =
		 * new Hashtable<String, String>(); List array = new ArrayList();
		 * Map<String, Object> jsonroot = new HashMap<String, Object>();
		 * jsonroot.put("id", ""); jsonroot.put("pId", "0");
		 * jsonroot.put("name", "角色列表"); jsonroot.put("checked", false);
		 * jsonroot.put("nocheck", true); jsonroot.put("open", true);
		 * jsonroot.put("icon", appInfoService.getAPP_BASE_NAME() +
		 * "/images/icons/icon042a1.gif"); if (!Lang.isEmpty(rolec)) { rolelist
		 * = (LinkedList<Sys_role>) sysRoleService.listByCnd(rolec); for (int j
		 * = 0; j < rolelist.size(); j++) { Sys_role roleobj = rolelist.get(j);
		 * Map<String, Object> jsonobj = new HashMap<String, Object>();
		 * jsonobj.put("id", String.valueOf(roleobj.getId()));
		 * jsonobj.put("pId", ""); jsonobj.put("name", roleobj.getName()); if
		 * (StringUtils
		 * .isNotBlank(Strings.sNull(hashrole.get(String.valueOf(roleobj
		 * .getId()))))) { if (self) { jsonobj.put("chkDisabled", true); }
		 * jsonobj.put("checked", true); } else { jsonobj.put("checked", false);
		 * } array.add(jsonobj);
		 * 
		 * } } array.add(jsonroot); for (int i = 0; i < unitlist.size(); i++) {
		 * // 得到单位列表 Sys_unit unitobj = unitlist.get(i); String unitid =
		 * unitobj.getId(); Condition c = Cnd.wrap("unitid='" + unitid +
		 * "' order by id"); list = (LinkedList) sysRoleService.listByCnd(c);
		 * String tunit = ""; int t = 1; hm.put(unitid, unitid); for (int k =
		 * unitid.length(); k > 4; k = k - 4) { // 计算单位的上级（递归） t = k / 4 - 1; if
		 * (t != 0) { tunit = unitid.substring(0, unitid.length() - 4 * t); }
		 * else { tunit = unitid; } hm.put(tunit, tunit); } String name = "";
		 * for (int j = 0; j < list.size(); j++) { // 得到单位列表下的角色 Sys_role
		 * roleobj = list.get(j); name = roleobj.getName(); Map<String, Object>
		 * jsonobj = new HashMap<String, Object>(); jsonobj.put("id",
		 * String.valueOf(roleobj.getId())); jsonobj.put("pId", unitid);
		 * jsonobj.put("name", name); if
		 * (!"".equals(Strings.sNull(hashrole.get(String
		 * .valueOf(roleobj.getId()))))) { jsonobj.put("checked", true); } else
		 * { jsonobj.put("checked", false); } array.add(jsonobj); } } String
		 * temp = ""; Map.Entry[] set =
		 * SortHashtable.getSortedHashtableByKey(hm); for (int i = 0; i <
		 * set.length; i++) { String key = set[i].getKey().toString(); Sys_unit
		 * su = sysUnitService.detailByName(String.valueOf(key)); if (su !=
		 * null) { String tempUid = String.valueOf(key); String tempName =
		 * su.getName(); if (tempUid.length() == 4) { temp = "0"; } else { temp
		 * = tempUid.substring(0, tempUid.length() - 4); } Map<String, Object>
		 * jsonobj = new HashMap<String, Object>(); jsonobj.put("id", tempUid);
		 * jsonobj.put("pId", temp); jsonobj.put("name", tempName.length() > 12
		 * ? tempName.substring(0, 12) + ".." : tempName);
		 * jsonobj.put("isParent", true); jsonobj.put("open", true);
		 * jsonobj.put("nocheck", true); jsonobj.put("icon",
		 * appInfoService.getAPP_BASE_NAME() + "/images/icons/icon042a1.gif");
		 * array.add(jsonobj); } } req.setAttribute("str", Json.toJson(array));
		 */
	}
	...
	public Message updateInfo(@Attr(Webs.ME) Sys_user olduser, @Param("..") Sys_user user, @Param("password2") String pass, @Param("oldpassword") String oldpassword, HttpServletRequest req) {
		...
		if (sysUserService.checkPwd(olduser, oldpassword)) {
			...
			olduser.setPassword(hashedPasswordBase64);
